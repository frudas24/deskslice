...
func NormToAbsPresetup(xn, yn float64, m monitor.Monitor) (int, int) {
	xn = clamp01(xn)
	yn = clamp01(yn)
	return m.X + int(math.Round(xn*float64(m.W))), m.Y + int(math.Round(yn*float64(m.H)))
}
...
func NormToAbsRun(xn, yn float64, plugin calib.Rect) (int, int) {
	xn = clamp01(xn)
	yn = clamp01(yn)
	return plugin.X + int(math.Round(xn*float64(plugin.W))), plugin.Y + int(math.Round(yn*float64(plugin.H)))
}
